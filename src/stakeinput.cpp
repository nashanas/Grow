...
bool CZGrowStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    if (!pwallet->CreateZGROWOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zGROW output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zGROW", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZGROWOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zGROW output", __func__);
...
//!GROW Stake
bool CGrowStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CGrowStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CGrowStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CGrowStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CGrowStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CGrowStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CGrowStake::GetUniqueness()
{
...
CBlockIndex* CGrowStake::GetIndexFrom()
{
    uint256 hashBlock = 0;
    CTransaction tx;
