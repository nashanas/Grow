...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "grow" is a composite category enabling all GROW-related debug output
            if (ptrCategory->count(string("grow"))) {
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "grow.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty grow.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override grow.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "growd.pid"));
