...
std::set<std::string> ZCstlControlDialog::setSelectedMints;
std::set<CMintMeta> ZCstlControlDialog::setMints;

ZCstlControlDialog::ZCstlControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZCstlControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZCstlControlDialog::~ZCstlControlDialog()
{
    delete ui;
}

void ZCstlControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZCstlControlDialog::updateList()
{
...
void ZCstlControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZCstlControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZCstl_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZCstlControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZCstlControlDialog::GetSelectedMints()
{
...
void ZCstlControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
