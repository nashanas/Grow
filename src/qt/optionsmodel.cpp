...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::GROW);
    ...
    if (!settings.contains("nAnonymizeGrowAmount"))
        settings.setValue("nAnonymizeGrowAmount", 1000);

    nAnonymizeGrowAmount = settings.value("nAnonymizeGrowAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeGrowAmount"))
        SoftSetArg("-anonymizegrowamount", settings.value("nAnonymizeGrowAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in grow.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeGrowAmount:
            return QVariant(nAnonymizeGrowAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeGrowAmount:
            nAnonymizeGrowAmount = value.toInt();
            settings.setValue("nAnonymizeGrowAmount", nAnonymizeGrowAmount);
            emit anonymizeGrowAmountChanged(nAnonymizeGrowAmount);
            break;
        case CoinControlFeatures:
