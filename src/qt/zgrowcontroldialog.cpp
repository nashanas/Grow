...
#include "ui_zgrowcontroldialog.h"
...
std::set<std::string> ZGrowControlDialog::setSelectedMints;
std::set<CMintMeta> ZGrowControlDialog::setMints;

bool CZGrowControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZGrowControlDialog::COLUMN_DENOMINATION || column == ZGrowControlDialog::COLUMN_VERSION || column == ZGrowControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZGrowControlDialog::ZGrowControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZGrowControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZGrowControlDialog::~ZGrowControlDialog()
{
    delete ui;
}

void ZGrowControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGrowControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZGrowControlWidgetItem* itemDenom(new CZGrowControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZGrowControlWidgetItem *itemMint = new CZGrowControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zgrowTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zgrowTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zGROW.";
            else if (!mint.isSeedCorrect)
                strReason = "The zGROW seed used to mint this zGROW is not the same as currently hold in the wallet";
            else
...
void ZGrowControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGrowControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZGrow_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGrowControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZGrowControlDialog::GetSelectedMints()
{
...
void ZGrowControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
