                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zGROW ought to be enough for anybody." - Bill Gates, 2017
    ui->zGROWpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGROWSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGROWpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGROW_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGROW is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zGROW, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGROW...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGROW in ") +
...
void PrivacyDialog::on_pushButtonSpendzGROW_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGROW is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zGROW, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zGROW
        sendzGROW();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGROW
    sendzGROW();
}

void PrivacyDialog::on_pushButtonZGrowControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZGrowControlDialog* zGrowControl = new ZGrowControlDialog(this);
    zGrowControl->setModel(walletModel);
    zGrowControl->exec();
}

void PrivacyDialog::setZGrowControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGrowSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGROW()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Grow Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGROWpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGROW is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GROW </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGROW</b>";
    ...
    // use mints from zGROW selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGrowControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZGrowControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zGROW was spent successfully update the addressbook with the label
    ...
    // Clear zgrow selector in case it was used
    ZGrowControlDialog::setSelectedMints.clear();
    ui->labelzGrowSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGROW Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Grow, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGROW Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zgrowTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGROW </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGROW "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGROW "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zGROW "));
    ui->labelzGROWAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zGROW </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zGROW </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zGROW </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzGROWSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzGROW->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zGROW
        ui->pushButtonMintzGROW->setEnabled(false);
        ui->pushButtonMintzGROW->setToolTip(tr("zGROW is currently disabled due to maintenance."));

        // Spend zGROW
        ui->pushButtonSpendzGROW->setEnabled(false);
        ui->pushButtonSpendzGROW->setToolTip(tr("zGROW is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zGROW
        ui->pushButtonMintzGROW->setEnabled(true);
        ui->pushButtonMintzGROW->setToolTip(tr("PrivacyDialog", "Enter an amount of GROW to convert to zGROW", 0));

        // Spend zGROW
        ui->pushButtonSpendzGROW->setEnabled(true);
        ui->pushButtonSpendzGROW->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
