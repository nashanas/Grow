...
CzGROWWallet::CzGROWWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zgrow seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZGROWSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZGROWSeed_deprecated()) {
                    LogPrintf("%s: Updated zGROW seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zgrow seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zgrow wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzGROWWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZGROWCount(nCountLastUsed);
    else if (!walletdb.ReadZGROWCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzGROWWallet::Lock()
{
    seedMaster = 0;
}

void CzGROWWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzGROWWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZGROW(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzGROWWallet::LoadMintPoolFromDB()
{
...
void CzGROWWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzGROWWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzGROWWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zgrowTracker->HasPubcoinHash(pMint.first)) {
...
bool CzGROWWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZGROW(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zgrowTracker which also adds to database
    pwalletMain->zgrowTracker->Add(dMint, true);
...
void CzGROWWallet::SeedToZGROW(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzGROWWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzGROWWallet::GenerateDeterministicZGROW(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzGROWWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzGROWWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzGROWWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
