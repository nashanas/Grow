...
CzGROWTracker::CzGROWTracker(std::string strWalletFile)
{
...
CzGROWTracker::~CzGROWTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzGROWTracker::Init()
{
...
bool CzGROWTracker::Archive(CMintMeta& meta)
{
...
bool CzGROWTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzGROWTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzGROWTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzGROWTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzGROWTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzGROWTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzGROWTracker::GetSerialHashes()
{
...
CAmount CzGROWTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzGROWTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzGROWTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzGROWTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzGROWTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzGROWTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzGROWTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzGROWTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzGROWTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzGROWTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzGROWTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzGROWWallet* zGROWWallet)
{
    bool iszGROWWalletInitialized = (NULL != zGROWWallet);
    CMintMeta meta;
    ...
    if (! iszGROWWalletInitialized)
        zGROWWallet = new CzGROWWallet(strWalletFile);
    meta.isSeedCorrect = zGROWWallet->CheckSeed(dMint);
    if (! iszGROWWalletInitialized)
        delete zGROWWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzGROWTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzGROWTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzGROWTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzGROWTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzGROWTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzGROWTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzGROWWallet* zGROWWallet = new CzGROWWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zGROWWallet);
        }
        delete zGROWWallet;
        LogPrint("zero", "%s: added %d dzgrow from DB\n", __func__, listDeterministicDB.size());
    }
