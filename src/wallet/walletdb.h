...
public:
    ...
    bool WriteZGROWSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZGROWSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZGROWSeed_deprecated(uint256& seed);
    bool EraseZGROWSeed();
    bool EraseZGROWSeed_deprecated();

    bool WriteZGROWCount(const uint32_t& nCount);
    bool ReadZGROWCount(uint32_t& nCount);
