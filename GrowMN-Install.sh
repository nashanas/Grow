...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='grow.conf'
CONFIGFOLDER='/root/.grow'
COIN_DAEMON='/root/growd'
...
function compile_node() {
  ...
  chmod +x /root/grow*/* >/dev/null 2>&1
    cp /root/grow*/* /root >/dev/null 2>&1
  compile_error
  cp /root/grow*/* /usr/local/bin >/dev/null 2>&1
  chmod +x /usr/local/bin/grow* >/dev/null 2>&1
  compile_error
...
function create_key() {
  echo -e "Enter your ${YELLOW}GROW Masternode Private Key${NC}. "
...
function important_information() {
 echo
 echo -e "========================================================================================="
 echo -e "GROW Masternode is up and running listening on port ${YELLOW}$COIN_PORT${NC}."
 echo -e "Configuration file is: ${YELLOW}$CONFIGFOLDER/$CONFIG_FILE${NC}"
 echo -e "Start: ${YELLOW}service GROW start${NC}"
 echo -e "Stop: ${YELLOW}service GROW stop${NC}"
 ...
 echo -e "${YELLOW}./grow-cli getinfo${NC}"
 echo -e "${YELLOW}./grow-cli masternode status${NC}"
 echo -e "================================================================================================================================"
}
